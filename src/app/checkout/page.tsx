...
import { ShieldCheck, User, Globe, AlertCircle, Loader2, Image as ImageIcon } from "lucide-react";
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            {userData && (
              ...
              >
                <div className="relative w-20 h-20 bg-navy-dark rounded-lg overflow-hidden border border-white/10 flex items-center justify-center">
                  {imageLoading && (
                    <div className="absolute inset-0 flex items-center justify-center bg-navy-dark z-10">
                      <Loader2 className="animate-spin text-primary-gold/50" size={20} />
                    </div>
                  )}
                  {imageError ? (
                    <ImageIcon className="text-foreground/20" size={32} />
                  ) : (
                    <Image
                      src={userData.head}
                      alt="Skin"
                      fill
                      className={cn("object-contain transition-opacity duration-300", imageLoading ? "opacity-0" : "opacity-100")}
                      onLoad={() => setImageLoading(false)}
                      onError={() => {
                        setImageError(true);
                        setImageLoading(false);
                      }}
                      unoptimized={true} // Bypassing Next.js image optimization as a test/fallback
                    />
                  )}
                </div>
                <div>
